...
namespace Redking\Bundle\CoreRestBundle\Bridge\Nelmio\ApiDocBundle\Parser;
            ...
            switch ($bestType) {
                ...
                case 'choice':
                    if ($config->getOption('multiple')) {
                        ...
                        $parameters[$name]['saveAs'] = 'array';
                    }

                    if (($choices = $config->getOption('choices')) && is_array($choices) && count($choices)) {
                        $parameters[$name]['format'] = json_encode($choices, JSON_FORCE_OBJECT);
                    } elseif (($choiceList = $config->getOption('choice_list')) && $choiceList instanceof ChoiceListInterface) {
                        $class_name = explode('\\', $config->getOption('class'));
                        $parameters[$name]['source_collection'] = strtolower($class_name[count($class_name)-1]).'s';
                        $parameters[$name]['format'] = 'MongoDB ID (string)';
                        if (!isset($parameters[$name]['saveAs'])) {
                            $parameters[$name]['saveAs'] = 'string';
                        }
                ...
                case 'string':
                    $parameters[$name]['saveAs'] = 'string';
                    if ($config->getType()->getName() == 'email') {
                        $parameters[$name]['dataType'] = 'email';
                    }
                    $attrs = $config->getOption('attr');
                    if (isset($attrs['dataType'])) {
                        $parameters[$name]['dataType'] = $attrs['dataType'];
                    }
                break;
                case 'integer':
                    $parameters[$name]['saveAs'] = 'integer';
                    if ($config->getType()->getName() == 'timestamp') {
                        $parameters[$name]['dataType'] = 'date';
                    }
                break;
                case 'boolean':
                    $parameters[$name]['saveAs'] = 'boolean';
                break;
                case 'float':
                    $parameters[$name]['saveAs'] = 'float';
                break;
            }
        }
